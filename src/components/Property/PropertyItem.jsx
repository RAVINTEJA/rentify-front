...
const PropertyItem = ({ property }) => {
  ...
  return (
    <li className="p-6 bg-white rounded shadow-md">
      <h3 className="mb-2 text-xl font-bold text-primary">{property.title ? property.title : "Mansion" }</h3>
      <p className="mb-2 text-textSecondary">{property.description ? property.description :"Great Place" }</p>
      <p className="mb-2 text-textSecondary">Location: {property.place}</p>
      ...
      <p className="mb-2 text-textSecondary">Rent Price: {property.rentPrice}</p>
    ...
    <img src={property.images[0]}  className="object-cover w-full h-48 mb-4 rounded" />
      <button onClick={likeProperty} className="p-2 mb-2 text-white rounded bg-secondary">Likes {likes}</button>
      <button onClick={expressInterest} className="p-2 mb-2 text-white rounded bg-accent">I&apos;m Interested</button>
      {sellerDetails &&  (
        <PrivateRoute>
